
import Link from 'next/link';
import React from 'react';

const PortfolioDetailsArea = () => {
  return (
    <>
      <div className="lonyo-section-padding9">
        <div className="container">
          <div className="lonyo-integration-d-wrap">
            <div className="lonyo-section-title">
              <div className="lonyo-p-d-thumb" data-aos="fade-up" data-aos-duration="500">
                <img src="assets/images/portfolio/p7.png" alt="" />
              </div>
              <div className="lonyo-p-d-data-wrap">
                <div className="lonyo-p-d-data-text">
                  <p>Client:</p>
                  <h4>XYZ Company</h4>
                </div>
                <div className="lonyo-p-d-data-text">
                  <p>Services:</p>
                  <h4>Web Design</h4>
                </div>
                <div className="lonyo-p-d-data-text">
                  <p>Duration:</p>
                  <h4>2 Weeks</h4>
                </div>
                <div className="lonyo-p-d-data-text">
                  <p>Websit:</p>
                  <Link className="title-btn" href="/single-portfolio">
                    <h4>Live preview</h4>
                    <svg width="18" height="16" viewBox="0 0 18 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M17.7796 8.53108L11.03 15.2807C10.8893 15.4214 10.6984 15.5005 10.4994 15.5005C10.3004 15.5005 10.1095 15.4214 9.96882 15.2807C9.8281 15.14 9.74904 14.9491 9.74904 14.7501C9.74904 14.5511 9.8281 14.3602 9.96882 14.2195L15.4388 8.75045H0.749958C0.551057 8.75045 0.360302 8.67143 0.219658 8.53079C0.0790134 8.39014 0 8.19939 0 8.00049C0 7.80159 0.0790134 7.61083 0.219658 7.47019C0.360302 7.32954 0.551057 7.25053 0.749958 7.25053H15.4388L9.96882 1.78146C9.8281 1.64074 9.74904 1.44988 9.74904 1.25086C9.74904 1.05185 9.8281 0.860991 9.96882 0.720268C10.1095 0.579545 10.3004 0.500488 10.4994 0.500488C10.6984 0.500488 10.8893 0.579545 11.03 0.720268L17.7796 7.46989C17.8494 7.53954 17.9047 7.62225 17.9424 7.7133C17.9802 7.80434 17.9996 7.90193 17.9996 8.00049C17.9996 8.09904 17.9802 8.19663 17.9424 8.28768C17.9047 8.37872 17.8494 8.46143 17.7796 8.53108Z" fill="#142D6F" />
                      <path d="M17.7796 8.53108L11.03 15.2807C10.8893 15.4214 10.6984 15.5005 10.4994 15.5005C10.3004 15.5005 10.1095 15.4214 9.96882 15.2807C9.8281 15.14 9.74904 14.9491 9.74904 14.7501C9.74904 14.5511 9.8281 14.3602 9.96882 14.2195L15.4388 8.75045H0.749958C0.551057 8.75045 0.360302 8.67143 0.219658 8.53079C0.0790134 8.39014 0 8.19939 0 8.00049C0 7.80159 0.0790134 7.61083 0.219658 7.47019C0.360302 7.32954 0.551057 7.25053 0.749958 7.25053H15.4388L9.96882 1.78146C9.8281 1.64074 9.74904 1.44988 9.74904 1.25086C9.74904 1.05185 9.8281 0.860991 9.96882 0.720268C10.1095 0.579545 10.3004 0.500488 10.4994 0.500488C10.6984 0.500488 10.8893 0.579545 11.03 0.720268L17.7796 7.46989C17.8494 7.53954 17.9047 7.62225 17.9424 7.7133C17.9802 7.80434 17.9996 7.90193 17.9996 8.00049C17.9996 8.09904 17.9802 8.19663 17.9424 8.28768C17.9047 8.37872 17.8494 8.46143 17.7796 8.53108Z" fill="#142D6F" />
                    </svg>
                  </Link>
                </div>
              </div>
            </div>
            <div className="lonyo-default-content pb-35">
              <h2>Project description:</h2>
              <p>BudgetEase is an innovative personal finance app designed to simplify money management for individuals and families. With its intuitive interface and powerful features, BudgetEase helps users gain control over their finances, track spending, and achieve their financial goals effortlessly.</p>
            </div>
            <div className="lonyo-default-content pb-35">
              <h2>Key features:</h2>
              <div className="text text2">
                <p><span>1. Expense Tracking:</span> Automatically categorize and monitor spending to identify areas where you can save.</p>
                <p><span>2. Smart Budgeting:</span> Set customizable budgets and receive real-time updates to stay on track.</p>
                <p><span>3. Savings Goals:</span> Create and manage goals to build your emergency fund or save for big purchases.</p>
                <p><span>4. Bill Reminders:</span> Never miss a due date with timely bill alerts and tracking.</p>
                <p><span>5. Financial Insights:</span> Access detailed reports and charts to understand your financial habits.</p>
              </div>
            </div>
            <div className="lonyo-default-content pb-35">
              <h2>Mission:</h2>
              <p>BudgetEase is dedicated to empowering users to take control of their financial future through easy-to-use tools, personalized insights, and automation that simplifies money management.</p>
            </div>
            <div className="lonyo-default-content border-b">
              <h2>Target audience:</h2>
              <p>BudgetEase is perfect for individuals, families, and young professionals looking for a stress-free way to budget, save, and achieve financial stability. With BudgetEase, managing your money has never been easier!</p>
            </div>
          </div>
          <div className="lonyo-section-title center">
            <h2>We have several projects to show you</h2>
          </div>
          <div className="row">
            <div className="col-lg-5">
              <div className="lonyo-p-thumb thumb2" data-aos="fade-up" data-aos-duration="700">
                <img className="w100" src="assets/images/portfolio/p5.png" alt="" />
                <div className="lonyo-p-data-wrap wrap2">
                  <div className="lonyo-p-content">
                    <h4>DebtFreePath</h4>
                    <ul>
                      <li><Link href="/portfolio">Creative</Link></li>
                      <li><Link href="/portfolio">App</Link></li>
                    </ul>
                  </div>
                  <div className="lonyo-p-title-btn">
                    <Link className="title-btn" href="/portfolio">Learn more
                      <svg width="18" height="16" viewBox="0 0 18 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M17.7796 8.53108L11.03 15.2807C10.8893 15.4214 10.6984 15.5005 10.4994 15.5005C10.3004 15.5005 10.1095 15.4214 9.96882 15.2807C9.8281 15.14 9.74904 14.9491 9.74904 14.7501C9.74904 14.5511 9.8281 14.3602 9.96882 14.2195L15.4388 8.75045H0.749958C0.551057 8.75045 0.360302 8.67143 0.219658 8.53079C0.0790134 8.39014 0 8.19939 0 8.00049C0 7.80159 0.0790134 7.61083 0.219658 7.47019C0.360302 7.32954 0.551057 7.25053 0.749958 7.25053H15.4388L9.96882 1.78146C9.8281 1.64074 9.74904 1.44988 9.74904 1.25086C9.74904 1.05185 9.8281 0.860991 9.96882 0.720268C10.1095 0.579545 10.3004 0.500488 10.4994 0.500488C10.6984 0.500488 10.8893 0.579545 11.03 0.720268L17.7796 7.46989C17.8494 7.53954 17.9047 7.62225 17.9424 7.7133C17.9802 7.80434 17.9996 7.90193 17.9996 8.00049C17.9996 8.09904 17.9802 8.19663 17.9424 8.28768C17.9047 8.37872 17.8494 8.46143 17.7796 8.53108Z" fill="#142D6F" />
                        <path d="M17.7796 8.53108L11.03 15.2807C10.8893 15.4214 10.6984 15.5005 10.4994 15.5005C10.3004 15.5005 10.1095 15.4214 9.96882 15.2807C9.8281 15.14 9.74904 14.9491 9.74904 14.7501C9.74904 14.5511 9.8281 14.3602 9.96882 14.2195L15.4388 8.75045H0.749958C0.551057 8.75045 0.360302 8.67143 0.219658 8.53079C0.0790134 8.39014 0 8.19939 0 8.00049C0 7.80159 0.0790134 7.61083 0.219658 7.47019C0.360302 7.32954 0.551057 7.25053 0.749958 7.25053H15.4388L9.96882 1.78146C9.8281 1.64074 9.74904 1.44988 9.74904 1.25086C9.74904 1.05185 9.8281 0.860991 9.96882 0.720268C10.1095 0.579545 10.3004 0.500488 10.4994 0.500488C10.6984 0.500488 10.8893 0.579545 11.03 0.720268L17.7796 7.46989C17.8494 7.53954 17.9047 7.62225 17.9424 7.7133C17.9802 7.80434 17.9996 7.90193 17.9996 8.00049C17.9996 8.09904 17.9802 8.19663 17.9424 8.28768C17.9047 8.37872 17.8494 8.46143 17.7796 8.53108Z" fill="#142D6F" />
                      </svg>
                    </Link>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-lg-7">
              <div className="lonyo-p-thumb thumb2 mb-0" data-aos="fade-up" data-aos-duration="900">
                <img className="w100" src="assets/images/portfolio/p6.png" alt="" />
                <div className="lonyo-p-data-wrap">
                  <div className="lonyo-p-content">
                    <h4>DebtFreePath</h4>
                    <ul>
                      <li><Link href="/portfolio">Creative</Link></li>
                      <li><Link href="/portfolio">App</Link></li>
                    </ul>
                  </div>
                  <div className="lonyo-p-title-btn">
                    <Link className="title-btn" href="/portfolio">Learn more
                      <svg width="18" height="16" viewBox="0 0 18 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M17.7796 8.53108L11.03 15.2807C10.8893 15.4214 10.6984 15.5005 10.4994 15.5005C10.3004 15.5005 10.1095 15.4214 9.96882 15.2807C9.8281 15.14 9.74904 14.9491 9.74904 14.7501C9.74904 14.5511 9.8281 14.3602 9.96882 14.2195L15.4388 8.75045H0.749958C0.551057 8.75045 0.360302 8.67143 0.219658 8.53079C0.0790134 8.39014 0 8.19939 0 8.00049C0 7.80159 0.0790134 7.61083 0.219658 7.47019C0.360302 7.32954 0.551057 7.25053 0.749958 7.25053H15.4388L9.96882 1.78146C9.8281 1.64074 9.74904 1.44988 9.74904 1.25086C9.74904 1.05185 9.8281 0.860991 9.96882 0.720268C10.1095 0.579545 10.3004 0.500488 10.4994 0.500488C10.6984 0.500488 10.8893 0.579545 11.03 0.720268L17.7796 7.46989C17.8494 7.53954 17.9047 7.62225 17.9424 7.7133C17.9802 7.80434 17.9996 7.90193 17.9996 8.00049C17.9996 8.09904 17.9802 8.19663 17.9424 8.28768C17.9047 8.37872 17.8494 8.46143 17.7796 8.53108Z" fill="#142D6F" />
                        <path d="M17.7796 8.53108L11.03 15.2807C10.8893 15.4214 10.6984 15.5005 10.4994 15.5005C10.3004 15.5005 10.1095 15.4214 9.96882 15.2807C9.8281 15.14 9.74904 14.9491 9.74904 14.7501C9.74904 14.5511 9.8281 14.3602 9.96882 14.2195L15.4388 8.75045H0.749958C0.551057 8.75045 0.360302 8.67143 0.219658 8.53079C0.0790134 8.39014 0 8.19939 0 8.00049C0 7.80159 0.0790134 7.61083 0.219658 7.47019C0.360302 7.32954 0.551057 7.25053 0.749958 7.25053H15.4388L9.96882 1.78146C9.8281 1.64074 9.74904 1.44988 9.74904 1.25086C9.74904 1.05185 9.8281 0.860991 9.96882 0.720268C10.1095 0.579545 10.3004 0.500488 10.4994 0.500488C10.6984 0.500488 10.8893 0.579545 11.03 0.720268L17.7796 7.46989C17.8494 7.53954 17.9047 7.62225 17.9424 7.7133C17.9802 7.80434 17.9996 7.90193 17.9996 8.00049C17.9996 8.09904 17.9802 8.19663 17.9424 8.28768C17.9047 8.37872 17.8494 8.46143 17.7796 8.53108Z" fill="#142D6F" />
                      </svg>
                    </Link>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default PortfolioDetailsArea;